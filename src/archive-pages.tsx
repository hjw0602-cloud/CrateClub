...
type SearchRelease = Release & { source?: 'apple' | 'musicbrainz'; externalId?: string; musicbrainzId?: string }
...
export function CreatePage({ catalog: releases }: { catalog: typeof import('./data').releases }) {
  ...
  const selectRelease = async (release: SearchRelease) => {
    ...
    if (release.externalId || release.musicbrainzId) {
      ...
      try {
        const response = await fetch(`${catalogApi}/catalog/import`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ source: release.source || 'musicbrainz', externalId: release.externalId || release.musicbrainzId }) })
