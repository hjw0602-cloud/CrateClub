...
export function LandingPage() {
  ...
  return <div className="archive-landing">
    <section className="landing-hero">
      <div className="landing-copy"><span>DIG. COLLECT. DISPLAY.</span><h1>MAKE YOUR CRATE.</h1><p>좋아하는 앨범 9장을 고르고,<br />나만의 CRATEPRINT로 남기세요.</p><div><Link className="primary-btn" to="/create">지금 만들기 <ArrowRight /></Link><a className="outline-btn" href="#templates">템플릿 보기</a></div><small>미리보기까지 로그인 없이 바로 시작</small></div>
...
export function CreatePage({ catalog: releases }: { catalog: typeof import('./data').releases }) {
  ...
  return <div className="create-page section-wrap">
    <div className="create-workspace"><section className={`mobile-live-preview ${mobilePreviewOpen ? 'open' : 'collapsed'}`}><header><div><span>LIVE PREVIEW · {activeSlot + 1}번</span><b>{activeAlbum.title}</b><small>{activeAlbum.artist}</small></div><button onClick={() => setMobilePreviewOpen(value => !value)} aria-label={mobilePreviewOpen ? '미리보기 접기' : '미리보기 펼치기'}>{mobilePreviewOpen ? <ChevronUp /> : <ChevronDown />}</button></header>{mobilePreviewOpen && <div className="mobile-preview-canvas"><CrateprintPreview board={board} catalog={availableReleases} activeAlbumIndex={activeSlot} onAlbumSelect={setActiveSlot} /></div>}</section><section className="create-controls">
      ...
      <div className="create-control-block"><StepTitle index="02" title="앨범 선택" copy="바꿀 번호를 누르면 그 자리에서 바로 검색하고 교체할 수 있습니다." /><div className="active-slot-label"><span>{activeSlot + 1}번 앨범 편집 중</span><small>{availableReleases.length}개 저장됨 · Last.fm 전체 검색</small></div><div className="album-slot-picker inline-search">{albumIds.map((id,index) => { const album = availableReleases.find(item => item.id === id) || availableReleases[0]; const active = activeSlot === index; return <article className={active ? 'active' : ''} key={`${id}-${index}`}><button onClick={() => setActiveSlot(index)}><b>{index + 1}</b><img src={album.cover} alt="" /><span>{album.title}<small>{album.artist}</small></span></button>{active && <div className="slot-search-panel"><label className="album-search"><Search /><input value={query} onChange={event => setQuery(event.target.value)} placeholder={`${index + 1}번에 넣을 앨범 또는 아티스트 검색`} autoFocus /></label>{searching && <p className="album-search-status">Last.fm에서 검색 중...</p>}{searchError && <p className="album-search-error">{searchError}</p>}<div className="album-results">{visibleReleases.length ? visibleReleases.map(release => <article key={release.id}><img src={release.cover} alt="" /><div><b>{release.title}</b><small>{release.artist}{release.source === 'lastfm' ? ' · Last.fm' : ''}</small></div><button disabled={importingId === release.id} onClick={() => selectRelease(release)}><Plus /> {importingId === release.id ? '저장 중' : `${index + 1}번에 넣기`}</button></article>) : !searching && <p className="album-results-empty">검색 결과가 없습니다.</p>}</div></div>}</article> })}</div></div>
